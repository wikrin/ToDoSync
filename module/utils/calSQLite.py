...
#将字典key，value分别打包为元组，再将这两个元组打包成一个列表然后添加进列表数据结构：列表[列表[元组(), 元组()],列表[元组(), 元组()], ...]
                self.__key_value:list = [[tuple(sqldict), tuple(sqldict.values())] for sqldict in rule]
        ...
        elif out == 'IN' and type(rule) == dict and type(rule[list(rule.keys())[0]]) == dict:
            self.__key_value:list = [[("KEY", inc), (key, rule[inc][key])] for inc in rule for key in rule[inc]]

            return self.__key_value
        
        elif out == 'UP' and type(rule) == dict and type(rule[list(rule.keys())[0]]) == dict:
            self.key_value:list = [{inc : (key, rule[inc][key])} for inc in rule for key in rule[inc]]
    ...
    def add(self, rule, method:str=('INSERT'), table:str=('data')) -> bool: #传入参数，data,要处理的数据：list或dict类型，method,使用的方法:INSERT 插入，REPLACE 替换，table:要操作的表
        key_value = self.DDtO(rule,'IN')
        for tup in key_value:
            with self.conn:
                try:
                    self.cur.execute(f'''{method} INTO {table} {tup[0]} VALUES {tup[1]}''') #
                #     sql['epID'],
                #     sql['subject'],
                #     sql['calID'],
                #     sql['todoID'],
                ...
                #     0))
                    ...
                    print(tup)
                    if self.initupdate('init', (tup[0][1],tup[1][1]), [(tup[0][0], tup[1][0])]):
                        continue
                    else:
                        raise
        return True
...
#rule 
    def initupdate(self, table:str, col_value:tuple, where:list[tuple]=[('epID', 0)]) -> bool:
    ...
    def select(self, table:str, column:list[str]=['MICROSOFT'], where:list[tuple]=[()]) -> list:
