...
class Bangumi():
    ...
    def updata(self):
        ...
        list = sql.select('data', column=['epID'], where=[('status', 'completed'), ('type', 0)])
        num:list = [id[0] for id in list]
        for id in num:
            try:
                resource = requests.put(f"https://api.bgm.tv/v0/users/-/collections/-/episodes/{id}", headers=__headers, data=json.dumps(__body))
                if resource.status_code == 204:
                    sql.initupdate(table='data', col_value=('type',self.TYPE), where=[('epID', id)])
